...
class ProxyServer {
  ...
  * [Symbol.iterator]() {
  ...
  _handleConnect(req, downstream, head) {
    ...
    if (this._bypass(req.url)) {
      this._forwardConnect(opt.hostname, opt.port, req, downstream, head, false);
      return;
    }
    if (this._config.https && this._checkHttpsHosts(opt.hostname, opt.port)) {
      this._forwardConnect(
        ...
        req, downstream, head, false);
      return;
    }
    this._forwardConnect(opt.hostname, opt.port, req, downstream, head, true);
  }
  ...
  _bypass(host) {
    host = host.indexOf(':') >=0 ? host : (host + ':80');
    return bypass.indexOf(host) >= 0;
  }
