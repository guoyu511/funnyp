...
function InspectorCtrl($http, servers, $routeParams, $scope, $sce, dialogs) {
  ...
  this.hasAnyHeader = function (headers) {
    if (!headers) {
      return false;
    }
...
angular.module(moduleName, ['ngSanitize'])
  ...
  .config(($filterProvider) => {
    $filterProvider.register('trustAsResourceUrl', $sce =>{
      return (val) => {
  ...
  .config(($routeProvider, $httpProvider) => {
    ...
    $httpProvider.defaults.headers.common = {
      'x-funny-proxy' :'direct'
    };
  })
  .run(($location, $q, $rootScope) => {
    let channelId = '', socket;
